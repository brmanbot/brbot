...
def setup(bot):
    ...
    async def vid(inter, colour: str, name: str, url: str):
        ...
        if colour.lower() not in valid_colours:
            await inter.send(
                "Invalid colour. Please use `Green`, `Red`, or `Yellow`.",
                ephemeral=True
            )
            return
        ...
        if not (url.startswith("https://cdn.discordapp.com/attachments/") or
            ...
            await inter.response.send_message(
                "Discord video URLs only loser.", ephemeral=True
            )
            return
        ...
        if short_url is None:
            await inter.response.send_message(
                "Error creating short URL", ephemeral=True
            )
            return
        ...
        async with aiosqlite.connect("videos.db") as db:
            try:
                ...
                await inter.response.send_message(
                    f"An error occurred while saving the video: {e}", ephemeral=True
                )
