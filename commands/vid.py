...
async def vid(inter, colour: str, name: str, url: str):
    ...
    if colour.lower() not in valid_colours:
        await inter.send("Invalid colour. Please use `Green`, `Red`, or `Yellow`.")
        return
    
    if not (url.startswith("https://cdn.discordapp.com/attachments/") or url.startswith("https://media.discordapp.net/attachments/")):
        await inter.response.send_message("Discord video URLs only loser.")
        return
    
    short_url = await shorten_url(url)
    if short_url is None:
        await inter.response.send_message("Error creating short URL")
        return
    ...
    async with aiosqlite.connect("videos.db") as db:
        try:
            ...
            for result in results:
                if result[1] == name:
                    await inter.response.send_message(f"An entry with the same name `{name}` already exists in the database. Please use a different name.")
                    return
                if result[2] == short_url or result[4] == url:
                    await inter.response.send_message(f"An entry with the same URL `{url}` already exists in the database. Please use a different URL or video.")
                    return
            ...
            if "NOT NULL constraint failed" in str(e):
                ...
                await inter.response.send_message(f"An error occurred while saving the video: {e}. Column `{column_name}` has a NULL value.")
            else:
                await inter.response.send_message(f"An integrity error occurred while saving the video: {e}")
        except aiosqlite.Error as e:
            await inter.response.send_message(f"An error occurred while saving the video: {e}")
